...
export function entityLinks(host: HTMLElement, hass: HomeAssistant | undefined, entity: string | null | undefined,
  label = "Open entity", deviceId?: string | null, deviceLabel = "Open device", nativeButton = false) {
  ...
  const open = () => host.dispatchEvent(new CustomEvent("hass-more-info", {
    detail: { entityId: entity }, bubbles: true, composed: true,
  }));
  return html`${known ? nativeButton
    ? html`<button type="button" @click=${open}>${label}</button>`
    : html`<ha-button @click=${open}>${label}</ha-button>` : nothing}
