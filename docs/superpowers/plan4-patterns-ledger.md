...
Final: scoped re-review clean (A–H addressed). Parked minors for the next hygiene pass: p_on non-monotonic when a bucket is almost entirely unknown (add a min-observed-minutes floor); coordinator↔patterns reference cycle across reloads; SAVE_DELAY name reused with two values; async_backfill last_on naming; late-arriving first light never triggers back-fill; no unknown row written at shutdown (outage minutes attributed to last state); lights count not re-published on registry change.
